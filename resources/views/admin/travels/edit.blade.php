...
<div class="upload-progress-modal" id="uploadProgressModal" style="display: none;">
    <div class="upload-progress-card">
        <div class="progress-header">
            ...
            <div>
                <h3 class="progress-title" id="progressTitleText">Actualizando y Guardando Cambios...</h3>
                <p class="progress-subtitle" id="progressStatusText">Por favor espera mientras se cargan los archivos.</p>
            </div>
        </div>
...
<div class="admin-card glass" style="max-width: 850px; margin: 0 auto;">
    <div class="admin-card-title">
        ...
        Editar Detalles del Viaje: <span style="color: var(--insta-orange); margin-left: 0.5rem;">{{ $travel->title }}</span>
    </div>

    <form action="{{ route('admin.travels.update', $travel) }}" method="POST" enctype="multipart/form-data" id="travelEditForm">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 1.25rem;">
            <div class="form-group">
                ...
                <input type="text" name="location" id="location" class="form-input" value="{{ old('location', $travel->location) }}" placeholder="Ej. Cusco">
            </div>

            <div class="form-group">
                <label for="country" class="form-label">País</label>
                <input type="text" name="country" id="country" class="form-input" value="{{ old('country', $travel->country) }}" placeholder="Ej. Perú">
            </div>

            <div class="form-group">
                <label for="year" class="form-label">Año</label>
                <input type="text" name="year" id="year" class="form-input" value="{{ old('year', $travel->year ?? '2025') }}" placeholder="Ej. 2025">
            </div>

            <div class="form-group">
                <label for="travel_date" class="form-label">Fecha del Viaje</label>
                <input type="text" name="travel_date" id="travel_date" class="form-input" value="{{ old('travel_date', $travel->travel_date) }}" placeholder="Ej. 15 de Octubre, 2025">
            </div>
        </div>

        <div class="form-group">
            <label for="description" class="form-label">Resumen Breve (Aparece en la tarjeta) *</label>
            <textarea name="description" id="description" class="form-input" style="min-height: 90px;" required>{{ old('description', $travel->description) }}</textarea>
        </div>

        <div class="form-group">
            <label for="content" class="form-label">Bitácora Completa / Reseña Extendida (Página de detalles)</label>
            <textarea name="content" id="content" class="form-input" style="min-height: 150px; line-height: 1.7;" placeholder="Escribe la experiencia completa, reflexiones, ruta del viaje y detalles fotográficos...">{{ old('content', $travel->content) }}</textarea>
        </div>
        ...
        <div style="background: rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.08); padding: 1.75rem; border-radius: 18px; margin-bottom: 1.75rem;">
            ...
            <div class="media-type-selector">
                <div class="media-option-card {{ old('media_type', $travel->media_type ?? 'image') === 'image' ? 'active' : '' }}" id="cardMediaImage" onclick="selectMediaTypeCard('image')">
                    <input type="radio" name="media_type" value="image" {{ old('media_type', $travel->media_type ?? 'image') === 'image' ? 'checked' : '' }} style="display:none;" id="radioMediaImage">
                ...
                <div class="media-option-card {{ old('media_type', $travel->media_type) === 'video' ? 'active' : '' }}" id="cardMediaVideo" onclick="selectMediaTypeCard('video')">
                    <input type="radio" name="media_type" value="video" {{ old('media_type', $travel->media_type) === 'video' ? 'checked' : '' }} style="display:none;" id="radioMediaVideo">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label for="image" class="form-label" style="color: #ffffff;">
                        ...
                        Imagen de Fondo 100vh <span style="font-size: 0.72rem; color: var(--text-muted); text-transform: none;">(Máx. 15 MB)</span>
                    </label>

                    <div class="file-dropzone-custom">
                        <input type="file" name="image" id="image" class="file-hidden-input" accept="image/*" onchange="handleFileDropzoneSelect(this, 'imgFileName', previewImageLive)">
                        <div class="file-dropzone-label">
                            ...
                            <div class="dropzone-info-text">
                                ...
                                <span class="dropzone-file-name {{ $travel->image_path ? 'has-file' : '' }}" id="imgFileName">
                                    {{ $travel->image_path ? basename($travel->image_path) : 'Ningún archivo seleccionado' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    
                    <div class="preview-box-container" id="imagePreviewBox" style="{{ $travel->image_path ? 'display:block;' : 'display:none;' }}">
                        <img id="imagePreviewImg" src="{{ $travel->image_path ? asset($travel->image_path) : '' }}" alt="Previsualización de Imagen">
                        @if($travel->image_path)
                            <label style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.65rem; padding: 0.4rem 0.6rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); border-radius: 8px; color: #f87171; font-size: 0.82rem; cursor: pointer; font-weight: 600;">
                                <input type="checkbox" name="delete_image" value="1" style="accent-color: #ef4444; width: 16px; height: 16px; cursor: pointer;">
                                <i class="fa-solid fa-trash-can"></i> Eliminar Imagen de Fondo Actual
                            </label>
                        @endif
                    </div>
                </div>
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    <label for="video" class="form-label" style="color: #ffffff;">
                        ...
                        <span id="videoReqBadge" style="font-size: 0.72rem; color: var(--insta-orange); text-transform: none; font-weight: 700; display: {{ old('media_type', $travel->media_type) === 'video' && !$travel->video_path ? 'inline-block' : 'none' }};">(OBLIGATORIO)</span>
                    </label>

                    <div class="file-dropzone-custom">
                        <input type="file" name="video" id="video" class="file-hidden-input" accept="video/mp4,video/webm,video/ogg" onchange="handleFileDropzoneSelect(this, 'vidFileName', previewVideoLive)">
                        <div class="file-dropzone-label">
                            ...
                            <div class="dropzone-info-text">
                                ...
                                <span class="dropzone-file-name {{ $travel->video_path ? 'has-file' : '' }}" id="vidFileName">
                                    {{ $travel->video_path ? basename($travel->video_path) : 'Ningún archivo seleccionado' }}
                                </span>
                            </div>
                        </div>
                    </div>
                    
                    <div class="preview-box-container" id="videoPreviewBox" style="{{ $travel->video_path ? 'display:block;' : 'display:none;' }}">
                        <video id="videoPreviewVid" controls src="{{ $travel->video_path ? asset($travel->video_path) : '' }}"></video>
                        @if($travel->video_path)
                            <label style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.65rem; padding: 0.4rem 0.6rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); border-radius: 8px; color: #f87171; font-size: 0.82rem; cursor: pointer; font-weight: 600;">
                                <input type="checkbox" name="delete_video" value="1" style="accent-color: #ef4444; width: 16px; height: 16px; cursor: pointer;">
                                <i class="fa-solid fa-trash-can"></i> Eliminar Video Actual
                            </label>
                        @endif
                    </div>
                ...
                <div class="file-dropzone-custom">
                    ...
                    <div class="file-dropzone-label">
                        ...
                        <div class="dropzone-info-text">
                            ...
                            <span class="dropzone-file-name {{ $travel->audio_path ? 'has-file' : '' }}" id="audFileName">
                                {{ $travel->audio_path ? basename($travel->audio_path) : 'Ningún archivo seleccionado' }}
                            </span>
                        </div>
                    </div>
                </div>
                
                <div class="preview-box-container" id="audioPreviewBox" style="padding: 0.75rem; {{ $travel->audio_path ? 'display:block;' : 'display:none;' }}">
                    <audio id="audioPreviewAud" controls src="{{ $travel->audio_path ? asset($travel->audio_path) : '' }}"></audio>
                    @if($travel->audio_path)
                        <label style="display: flex; align-items: center; gap: 0.5rem; margin-top: 0.65rem; padding: 0.4rem 0.6rem; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.25); border-radius: 8px; color: #f87171; font-size: 0.82rem; cursor: pointer; font-weight: 600;">
                            <input type="checkbox" name="delete_audio" value="1" style="accent-color: #ef4444; width: 16px; height: 16px; cursor: pointer;">
                            <i class="fa-solid fa-trash-can"></i> Eliminar Pista de Música Actual
                        </label>
                    @endif
                </div>
            </div>
...
<script>
    ...
    function selectMediaTypeCard(type) {
        ...
        if (type === 'image') {
            ...
            if (videoInput) videoInput.required = !hasExistingVideo;
            if (videoReqBadge) videoReqBadge.style.display = hasExistingVideo ? 'none' : 'inline-block';
        }
    }
